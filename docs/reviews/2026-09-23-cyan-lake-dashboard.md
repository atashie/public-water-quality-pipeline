...
- It does not show a forecast, a probability, a toxin level, a pixel value, or a validated value. The 130 line is labeled as the EPA code's operationalization, not as a fact about any lake.
...
2. Whether "detecting", a median under the line with some pixel above zero, deserves its own color on the map. Today it is only visible in the median coloring and the popups.
...
| Simplify the left side of the map tab and put detailed and technical information into a FAQ at the top right, toggled off | fixed. Left panel: week, bloom line, coloring, pixel toggle, legend, counts, and a collapsed "More options" for coverage and bridging. FAQ button at the top right with 13 entries |
...
| Add a hover to inspect values | fixed. Each pixel file now carries the raw codes as a gray image. Moving the mouse over a lake's pixels reads the lake, the code, the converted index, and whether the pixel is counted, into a box on the map and under the Lake tab's small map. A headless probe at a point inside Apopka, Lake read code 178, wholly inside, counted |
...
| Estimate how much larger the page would be with a year of pixel images per lake, or whether that would explode it | corrected with evidence. The page itself would not grow at all. The pixel images are separate files loaded one lake at a time, and the outline, not the images, is 98 percent of those files today. A year of images was measured, not estimated: all 2,321 lakes over the newest 52 weekly files cost 281 MB encoded as today, or 53 MB with the codes stacked into one image per lake and colored in the browser, against 67 MB served today. Split into one file per lake per week, a lake's fetch stays what it is today, and the page and the up-front lake file do not grow. [Measurement 11](../measurements.md#11-a-year-of-per-lake-pixel-images-what-it-costs-in-bytes-2026-09-23). Whether to serve the year, and in which encoding, is an owner decision |

## Owner feedback, sixth round, 2026-09-23

| Feedback | Disposition |
|---|---|
| Have "show the newest week's pixels" on when the page opens | fixed. The box is ticked by default and the layer draws as soon as the map is zoomed past level 8. The deep link `px=0` turns it off. The FAQ and the README say so |
| American spelling throughout, color not colour | fixed. Every British spelling in the page, the review dashboard, the code, and the documents is replaced: color, gray, center, meter, liter, catalog, neighbor, labeled. Quoted source text and proper names keep their spelling: NASA's "Ocean Colour Land Imager", the Centre for Environmental Data Analysis, and its hostname. Preserved sources under `reference/` and result files under `outputs/` are untouched, as the rules require |
| Found while testing the pixel default | fixed. The `view`, `lake`, and `tab` deep links matched only at the start of the hash. They now match anywhere in it. Under the `probe` test hook the map's zoom animation is off, so a headless render reaches the zoomed view. Headless check: the Florida view at zoom 11 draws 15 overlays and reads Lake Apopka, code 180, and the same view with `px=0` draws none |

## Authorization

After the fourth round the owner approved the page and authorized commit and push. The commit was interrupted before it ran, and the owner then asked for the fifth round. On 2026-09-23 the owner approved the fifth round and authorized commit and push of the whole step. The owner authorized commit and push of the sixth round on 2026-09-23.
