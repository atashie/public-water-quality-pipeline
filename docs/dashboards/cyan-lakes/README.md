...
- **FAQ.** A button at the top right opens a dropdown that explains the data, the lake value, the index code, the bloom line, the colors, coverage, runs, the pixels, the names, currency, limits, and sources, in plain language. Build details such as file hashes are not on the page. They are in the stamped summary under `datasets/cyan/outputs/`.
- A view deep link `#view=<lat>,<lon>,<zoom>` opens the map at a place. The pixel layer is on by default, and `&px=0` turns it off.
...
| `data/pixels/<comid>.js` | The lake's pixels from the newest weekly file as a colored PNG in Web Mercator with its bounds, a gray PNG whose value is the raw code for the hover readout, and the lake's exact outline in WGS84 to 6 decimals. Pixels wholly inside the outline at full opacity, touched pixels faded, land gray, no data light gray. 2,321 files, 67 MB, ignored by git |
