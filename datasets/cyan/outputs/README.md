...
| `pixel-history-<stamp>.json` | `viz/estimate_pixel_history.py` | Bytes of the per-lake pixel images for the newest N weekly files, 52 by default, as the dashboard encodes them, per week, per lake, and in total, with two cheaper encodings measured beside them. The key `year_bytes` holds the totals over the N files |
| `zero-and-land-<stamp>.json` | `qaqc/measure_zero_and_land.py` | Weekly lake rows whose median or every valid pixel is code 0, and the lakes with land-coded interior pixels in every weekly file, with the coverage those allow |
| `coverage-by-year-<stamp>.json` | `qaqc/measure_coverage_by_year.py` | Per calendar year of weekly files, over all months and over May to October: files, lake-weeks, lake-weeks with a value, and the mean share of interior pixels valid |
